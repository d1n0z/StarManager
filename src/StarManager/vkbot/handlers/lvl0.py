...
async def stats(message: Message):
    ...
    avatar_path = Path(
        settings.service.path + f"src/StarManager/core/media/temp/{id}ava.jpg"
    )
...
async def event(message: Message):
    ...
    if user is not None and user.event_user is not None:
        hc = user.event_user.has_cases
        cr = hc + user.event_user.cases_opened
    else:
        hc, cr = 0, 0
    await messagereply(
        message,
        await messages.event(cr, hc),
