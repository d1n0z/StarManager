...
from StarManager.core.tables import Filters

from . import filtering


@dataclass
class CachedChatFiltersRow(BaseCachedModel):
    chat_id: int
    filters: Set[str]


CacheKey: TypeAlias = int
Cache: TypeAlias = Dict[CacheKey, CachedChatFiltersRow]


class ChatFiltersCache(BaseCacheManager):
    def __init__(self, cache: Cache):
        super().__init__()
        self._cache: Cache = cache
        self._dirty: Set[CacheKey] = set()
        self._matchers: Dict[CacheKey, filtering.Matcher] = {}
    ...
    def _build_matcher_for_key(self, key: CacheKey):
        ...
        norm_filters = {
            filtering.normalize_text(f) for f in cached.filters if f and f.strip()
        }
        ...
        matcher = filtering.Matcher(normalized_filters=norm_filters)
    ...
    async def initialize(self):
        async with self._lock:
            ...
            for row in await Filters.all():
                key = row.chat_id
                if key not in self._cache:
                    self._cache[key] = CachedChatFiltersRow(
    ...
    async def sync(self, batch_size: int = 200):
        ...
        for i in range(0, len(items), batch_size):
            ...
            existing_rows = await Filters.filter(chat_id__in=[key for key, _ in batch])
            existing_map: dict = {}
            ...
            for key, cached_filters in batch:
                ...
                for f in create_filters:
                    to_create.append(Filters(chat_id=chat_id, filter_=f))
    ...
    async def new_filter(self, key: CacheKey, filter_: str):
        ...
        async with self._lock:
            if key not in self._cache:
                self._cache[key] = CachedChatFiltersRow(chat_id=chat_id, filters=set())
...
class ChatFiltersManager(BaseManager):
    def __init__(self):
        super().__init__()
        self._cache: Cache = {}
        self.cache = ChatFiltersCache(self._cache)
    ...
    async def del_all_filters(self, chat_id: int):
        return await self.cache.del_all_filters(chat_id)

    async def get_all(self, chat_id: int):
        return await self.cache.get_all_filters(chat_id)

    def matches(self, chat_id: int, message: str):
        return self.cache.matches(chat_id, message)
