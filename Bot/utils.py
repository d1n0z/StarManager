...
async def deleteMessages(
    cmids: int | Iterable[int], chat_id: int
...
async def getIDFromMessage(
    message: str, reply: ForeignMessageMin | None, place: int = 2
) -> int:
    """
...
async def sendMessage(
    peer_ids: int | Iterable[int],
    msg: str | None = None,
    kbd: str | None = None,
    photo: str | None = None,
    disable_mentions: int = 1,
) -> list[MessagesSendUserIdsResponseItem] | int | bool:
    try:
        msgs = await api.messages.send(
            random_id=0,
            peer_ids=peer_ids,
            message=msg,
            keyboard=kbd,
    ...
    async with (await pool()).acquire() as conn:
        ...
        try:
            chatname = await api.messages.get_conversations_by_id(
                peer_ids=chat_id + 2000000000, group_id=GROUP_ID
            )
...
async def getUserMuteInfo(uid, chat_id, none=None) -> dict:
    ...
    if mute:
        return {
            "times": literal_eval(mute[0]),
            "causes": literal_eval(mute[1]),
            "names": literal_eval(mute[2]),
...
async def getXPTop(
    returnval="count", limit: int = 10, league: int = 1, users: list = None
) -> dict:
    async with (await pool()).acquire() as conn:
...
async def getUserDuelWins(uid, none=0):
    async with (await pool()).acquire() as conn:
        return (
            await conn.fetchval("select wins from duelwins where uid=$1", uid) or none
        )
...
async def turnChatSetting(chat_id, category, setting, alt=False):
    defaults = (
        SETTINGS_DEFAULTS[setting]
        if setting in SETTINGS_DEFAULTS
        else {"pos": SETTINGS()[category][setting]}
    )
    async with (await pool()).acquire() as conn:
        if not await conn.fetchval(
            "update settings set "
            + (
                f"pos2 = NOT COALESCE(pos2, {str(bool(SETTINGS_ALT()[category][setting]))})"
                if alt
                else "pos=not pos"
            )
...
async def punish(uid, chat_id, setting_id):
    ...
    if name in SETTINGS_POSITIONS["antispam"]:
        cause = (
            "Анти-спам (#AS"
            + str(list(SETTINGS_POSITIONS["antispam"].keys()).index(name) + 1)
            + ")"
        )
    else:
...
async def ischatPubic(chat_id, none=False):
    async with (await pool()).acquire() as conn:
        return (
            await conn.fetchval(
                "select isopen from publicchats where chat_id=$1", chat_id
            )
            or none
        )
...
def beautifyNumber(n):
    return "".join(
        (i + " ") if k % 3 == 2 and k != len(str(n)) - 1 else i
