...
async def join(message: MessageEvent):
    ...
    if cmd == "join" or (cmd == "rejoin" and not payload["activate"]):
        ...
        if (
            ...
            and await utils.getUserAccessLevel(bp, chat_id) < 7
        ):
            return
        ...
        if cmd == "join":
            try:
                await tgbot.send_message(
                    chat_id=TG_CHAT_ID,
                    message_thread_id=TG_NEWCHAT_THREAD_ID,
        ...
        await utils.editMessage(
            messages.start(), peer_id, message.conversation_message_id
        )
        return
    elif cmd == "rejoin" and payload["activate"]:
        if await utils.getUserAccessLevel(uid, chat_id) >= 7 or uid in [
            i.member_id
            for i in (
            ...
            await utils.editMessage(
                messages.rejoin_activate(), peer_id, message.conversation_message_id
            )
            return
...
@bl.raw_event(
    GroupEventType.MESSAGE_EVENT,
    MessageEvent,
    SearchPayloadCMD(["report_answer"], checksender=False),
)
async def report_answer(message: MessageEvent):
    ...
    await utils.editMessage(
        messages.report_answering(repid),
...
@bl.raw_event(
    GroupEventType.MESSAGE_EVENT,
    MessageEvent,
    SearchPayloadCMD(["report_ban"], checksender=False),
)
async def report_ban(message: MessageEvent):
    ...
    async with (await pool()).acquire() as conn:
        if not await conn.fetchval(
            "select exists(select 1 from reportban where uid=$1 and time=0)", uid
        ):
            if not await conn.fetchval(
    ...
    await utils.sendMessage(
        payload["uid"],
        messages.report_banned(
            message.user_id, await utils.getUserName(message.user_id)
        ),
    )
    await utils.editMessage(
        messages.report_ban(
            message.user_id,
            await utils.getUserName(message.user_id),
            repid,
            payload["uid"],
            await utils.getUserName(payload["uid"]),
            payload["text"],
        ),
        message.object.peer_id,
        message.conversation_message_id,
...
@bl.raw_event(
    GroupEventType.MESSAGE_EVENT,
    MessageEvent,
    SearchPayloadCMD(["report_delete"], checksender=False),
)
async def report_delete(message: MessageEvent):
    await utils.sendMessage(
        message.payload["uid"], messages.report_deleted(message.payload["repid"])
    )
    await utils.deleteMessages(
        message.conversation_message_id, message.peer_id - 2000000000
    )
...
async def premmenu(message: MessageEvent):
    ...
    await utils.editMessage(
        messages.premmenu(settings, prem),
        message.peer_id,
        message.conversation_message_id,
...
async def premmenu_turn(message: MessageEvent):
    ...
    if payload["setting"] == "tagnotif" and not (
        await utils.isMessagesFromGroupAllowed(uid)
    ):
        await utils.editMessage(
            messages.tagnotiferror(),
            message.peer_id,
            message.conversation_message_id,
        )
        return
    async with (await pool()).acquire() as conn:
        if not await conn.fetchval(
            ...
            await conn.execute(
                ...
                uid,
                payload["setting"],
                int(not PREMMENU_DEFAULT[payload["setting"]]),
            )
    prem = await utils.getUserPremium(uid)
    settings = await utils.getUserPremmenuSettings(uid)
    await utils.editMessage(
...
async def premmenu_action(message: MessageEvent):
    ...
    if deleted:
        ...
        return
    await utils.editMessage(
        messages.premmenu_action(setting), peer_id, message.conversation_message_id
    )
...
async def settings_menu_antispam(message: MessageEvent):
    ...
    if payload["setting"] == "msgs":
        chrs, msgs = [
            settings.get(i, (i, None, None, None, None))
...
async def settings(message: MessageEvent):
    ...
    if payload["cmd"] == "settings":
        ...
        await utils.editMessage(
            await messages.settings_category(category, settings, chat_id),
            peer_id,
            message.conversation_message_id,
            keyboard.settings_category(uid, category, settings),
        )
        return
    ...
    if setting not in SETTINGS_COUNTABLE:
        ...
        await utils.editMessage(
            await messages.settings_category(category, settings[category], chat_id),
            peer_id,
            message.conversation_message_id,
            keyboard.settings_category(uid, category, settings[category]),
        )
        return
    async with (await pool()).acquire() as conn:
    ...
    await utils.editMessage(
        messages.settings_change_countable(
            chat_id,
            setting,
        ...
        keyboard.settings_change_countable(
            uid, category, setting, settings, altsettings
        ),
    )
...
async def settings_change_countable(message: MessageEvent):
    ...
    if action == "set":
        if setting == "welcome":
            ...
            if w:
                await utils.editMessage(
                    messages.settings_countable_action(
                ...
                return
            await utils.editMessage(
                messages.settings_countable_action(action, setting),
                peer_id,
                message.conversation_message_id,
        ...
        await utils.editMessage(
            messages.settings_choose_punishment(),
            peer_id,
            message.conversation_message_id,
        ...
        await utils.editMessage(
            messages.settings_set_preset(category, setting),
            peer_id,
            message.conversation_message_id,
        ...
        await utils.editMessage(
            messages.settings_setlist(setting, action[3:-4]),
            peer_id,
            message.conversation_message_id,
...
async def settings_set_punishment(message: MessageEvent):
    ...
    if action in ["deletemessage", "kick", "", "warn"]:
        ...
        await utils.editMessage(
            messages.settings_set_punishment(action),
            peer_id,
            message.conversation_message_id,
            keyboard.settings_change_countable(
                uid,
                category,
                setting,
                await utils.getChatSettings(chat_id),
                await utils.getChatAltSettings(chat_id),
                True,
            ),
        )
        return
    ...
    await utils.editMessage(
        messages.settings_set_punishment_input(action),
        peer_id,
        message.conversation_message_id,
    )
    return
    ...
    if setting == "forwardeds":
        ...
        await utils.editMessage(
            messages.settings_exceptionlist(
                [
                    f"[id{i[0]}|{await utils.getUserName(i[0]) if i[0] > 0 else await utils.getGroupName(i[0])}]"
                    for i in lst
                ]
            ),
            peer_id,
...
async def settings_listaction(message: MessageEvent):
    ...
    await utils.editMessage(
        messages.settings_listaction_action(setting, action),
        peer_id,
        message.conversation_message_id,
    )
...
async def settings_set_welcome(message: MessageEvent):
    ...
    await utils.editMessage(
        messages.get(cmd), message.peer_id, message.conversation_message_id
    )
...
async def settings_unset_welcome(message: MessageEvent):
    ...
    await utils.editMessage(
        messages.settings_countable_action("set", "welcome"),
        peer_id,
        message.conversation_message_id,
...
async def nicklist(message: MessageEvent):
    ...
    res = res[:30]
    await utils.editMessage(
        messages.nlist(res, await api.users.get(user_ids=[i[0] for i in res])),
        peer_id,
        message.conversation_message_id,
...
async def page_nlist(message: MessageEvent):
    ...
    await utils.editMessage(
        messages.nlist(res, await api.users.get(user_ids=[i[0] for i in res]), page),
        peer_id,
        message.conversation_message_id,
...
async def nonicklist(message: MessageEvent):
    ...
    await utils.editMessage(
        messages.nnlist(await api.users.get(user_ids=members_uid)),
        peer_id,
        message.conversation_message_id,
...
async def punishlist_delall(message: MessageEvent):
    ...
    await utils.editMessage(
        messages.punishlist_delall_done(cmd.replace("list_delall", "")),
        peer_id,
        message.conversation_message_id,
    )
...
async def giveowner_no(message: MessageEvent):
    await utils.editMessage(
        messages.giveowner_no(), message.object.peer_id, message.conversation_message_id
    )
...
async def resetnick_accept(message: MessageEvent):
    ...
    await utils.editMessage(
        messages.resetnick_accept(uid, await utils.getUserName(uid)),
        peer_id,
        message.conversation_message_id,
    )
...
async def resetaccess_accept(message: MessageEvent):
    ...
    await utils.editMessage(
        messages.resetaccess_accept(uid, await utils.getUserName(uid), lvl),
        peer_id,
        message.conversation_message_id,
    )
...
async def resetaccess_disaccept(message: MessageEvent):
    await utils.editMessage(
        messages.resetaccess_disaccept(message.payload["lvl"]),
        message.object.peer_id,
        message.conversation_message_id,
    )
...
async def kick_nonick(message: MessageEvent):
    ...
    await utils.sendMessage(
        msg=messages.kickmenu_kick_nonick(
            uid,
            await utils.getUserName(uid),
            await utils.getUserNickname(uid, chat_id),
            kicked,
...
async def kick_nick(message: MessageEvent):
    ...
    await utils.sendMessage(
        msg=messages.kickmenu_kick_nick(
            uid,
            await utils.getUserName(uid),
            await utils.getUserNickname(uid, chat_id),
            kicked,
...
async def kick_banned(message: MessageEvent):
    ...
    await utils.sendMessage(
        msg=messages.kickmenu_kick_banned(
            uid,
            await utils.getUserName(uid),
            await utils.getUserNickname(uid, chat_id),
            kicked,
...
async def notification_status(message: MessageEvent):
    ...
    await utils.editMessage(
        messages.notification(name, snotif[0], ntime, snotif[1], snotif[2], turn_to),
        peer_id,
        message.conversation_message_id,
...
async def notification_text(message: MessageEvent):
    ...
    await utils.editMessage(
        messages.notification_changing_text(), peer_id, message.conversation_message_id
    )
...
async def notification_time(message: MessageEvent):
    await utils.editMessage(
        messages.notification_changing_time_choose(),
        message.object.peer_id,
        message.conversation_message_id,
...
async def notification_time_change(message: MessageEvent):
    ...
    if ctype == "single":
        await utils.editMessage(
            messages.notification_changing_time_single(),
            peer_id,
            message.conversation_message_id,
        )
    elif ctype == "everyday":
        await utils.editMessage(
            messages.notification_changing_time_everyday(),
            peer_id,
            message.conversation_message_id,
        )
    else:
        await utils.editMessage(
            messages.notification_changing_time_everyxmin(),
            peer_id,
            message.conversation_message_id,
        )
...
async def notification_tag(message: MessageEvent):
    await utils.editMessage(
        messages.notification_changing_tag_choose(),
        message.object.peer_id,
        message.conversation_message_id,
...
async def notification_tag_change(message: MessageEvent):
    ...
    await utils.editMessage(
        messages.notification(name, notif[0], notif[1], notif[2], ctype, notif[3]),
        peer_id,
        message.conversation_message_id,
...
async def notification_delete(message: MessageEvent):
    ...
    await utils.editMessage(
        messages.notification_delete(name), peer_id, message.conversation_message_id
    )
...
async def help(message: MessageEvent):
    ...
    for i in cmds:
        try:
            ...
            pass
    await utils.editMessage(
        messages.help(payload["page"], base),
        peer_id,
        message.conversation_message_id,
    ...
    if check == "ban":
        ...
        await utils.editMessage(
            messages.check_ban(
                ...
                await utils.getUserName(id),
                await utils.getUserNickname(id, chat_id),
                max(await utils.getUserBan(id, chat_id) - time.time(), 0),
                u_bans_names,
                ban_date,
                ban_from,
                ban_reason,
        ...
        await utils.editMessage(
            messages.check_mute(
                ...
                await utils.getUserName(id),
                await utils.getUserNickname(id, chat_id),
                max(await utils.getUserMute(id, chat_id) - time.time(), 0),
                u_mutes_names,
                mute_date,
                mute_from,
                mute_reason,
        ...
        await utils.editMessage(
            messages.check_warn(
                ...
                await utils.getUserName(id),
                await utils.getUserNickname(id, chat_id),
                await utils.getUserWarns(id, chat_id),
                u_warns_names,
                u_warns_dates,
                u_warns_names,
                u_warns_causes,
...
async def check_history(message: MessageEvent):
    ...
    if not int(payload["ie"]):
        await utils.sendMessageEventAnswer(
            message.event_id,
            uid,
            message.peer_id,
...
async def unpunish(message: MessageEvent):
    ...
    if u_acc <= await utils.getUserAccessLevel(id, chat_id) or not await haveAccess(
        cmd, chat_id, u_acc
    ):
    ...
    if cmd == "unmute":
        ...
        await utils.setChatMute(id, chat_id, 0)
        await utils.editMessage(
            messages.unmute(uname, unickname, uid, name, nickname, id),
            peer_id,
            message.conversation_message_id,
        ...
        async with (await pool()).acquire() as conn:
            if not await conn.fetchval(
                ...
                return
        await utils.editMessage(
            messages.unwarn(uname, unickname, uid, name, nickname, id),
            peer_id,
            message.conversation_message_id,
            keyboard.deletemessages(uid, [message.conversation_message_id]),
        )
        ...
        async with (await pool()).acquire() as conn:
            if not await conn.fetchval(
                ...
                return
        await utils.editMessage(
            messages.unban(uname, unickname, uid, name, nickname, id),
            peer_id,
            message.conversation_message_id,
            keyboard.deletemessages(uid, [message.conversation_message_id]),
        )
...
async def prefix_(message: MessageEvent):
    ...
    if not await utils.getUserPremium(uid):
        await utils.editMessage(
            messages.no_prem(), peer_id, message.conversation_message_id
        )
        return
    async with (await pool()).acquire() as conn:
        if (
        ...
        if cmd in ("prefix_add", "prefix_del"):
            ...
            await utils.editMessage(
                messages.get(cmd), peer_id, message.conversation_message_id
            )
...
async def timeout_settings(message: MessageEvent):
    ...
    async with (await pool()).acquire() as conn:
        if await conn.fetchval(
            ...
            return
    await utils.editMessage(
        messages.timeout_settings(),
        peer_id,
        message.conversation_message_id,
        keyboard.timeout_settings(
            message.user_id, await utils.getSilenceAllowed(chat_id)
        ),
    )
...
async def turnpublic(message: MessageEvent):
    ...
    await utils.editMessage(
        messages.chat(
            ...
            bjd,
            prefix,
            await utils.getChatName(chat_id),
            prem,
        ),
        peer_id,
        message.conversation_message_id,
...
async def import_(message: MessageEvent):
    ...
    if await utils.getUserAccessLevel(message.user_id, importchatid) < 7:
        await utils.editMessage(
            messages.import_notowner(),
            message.object.peer_id,
            message.conversation_message_id,
        )
        return
    await utils.editMessage(
        messages.import_(importchatid, await utils.getChatName(importchatid)),
        message.object.peer_id,
        message.conversation_message_id,
...
async def import_settings(message: MessageEvent):
    ...
    await utils.editMessage(
        messages.import_settings(
            importchid,
            await utils.getChatName(importchid),
            s := await utils.getImportSettings(message.user_id, importchid),
        ),
        message.object.peer_id,
        message.conversation_message_id,
...
async def import_start(message: MessageEvent):
    importchatid = message.payload["importchatid"]
    await utils.editMessage(
        messages.import_start(importchatid),
        message.object.peer_id,
        message.conversation_message_id,
    )
    ...
    await utils.editMessage(
        messages.import_end(importchatid),
        message.object.peer_id,
        message.conversation_message_id,
    )
...
async def blocklist_chats(message: MessageEvent):
    ...
    for chat in inf:
        msg += (
            f"➖ id {chat[0]} | {await utils.getChatName(chat[0])}"
...
async def blocklist(message: MessageEvent):
    ...
    for user in inf:
        msg += (
            f"➖ [id{user[0]}|{await utils.getUserName(user[0])}]"
...
async def bindlist(message: MessageEvent):
    ...
    await utils.editMessage(
        messages.bindlist(group, [(i[0], await utils.getChatName(i[0])) for i in res]),
        message.object.peer_id,
        message.conversation_message_id,
...
async def filterpunishments(message: MessageEvent):
    ...
    await utils.editMessage(
        messages.filter_punishments(pnt),
        message.object.peer_id,
        message.conversation_message_id,
...
async def filterlist(message: MessageEvent):
    ...
    await utils.editMessage(
        messages.filter_list(filters[25 * page : 25 * page + 25], page),
        message.object.peer_id,
        message.conversation_message_id,
...
async def buy(message: MessageEvent):
    ...
    await utils.editMessage(
        messages.buy_order(
            payment.metadata["pid"],
            uid,
            name,
            days,
            cost,
            (
                await api.utils.get_short_link(payment.confirmation.confirmation_url)
            ).short_url,
        ),
        message.object.peer_id,
        message.conversation_message_id,
