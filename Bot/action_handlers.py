...
async def action_handle(event: MessageNew) -> None:
    ...
    if (ban := await getUserBan(uid, chat_id)) > time.time():
        await sendMessage(event.peer_id, messages.kick_banned(
    ...
    if id:
        ...
        async with (await pool()).acquire() as conn:
            if s := await conn.fetchrow(
                ...
                m = await sendMessage(event.peer_id, welcome[0].replace(
                    '%name%', f"[id{uid}|{await getUserName(uid) if u_nickname is None else u_nickname}]"),
                                      keyboard.welcome(welcome[1], welcome[2]), welcome[3])
                if s[1]:
                    lw = await conn.fetchval(
                        'delete from welcomehistory where chat_id=$1 returning cmid', chat_id)
                    await deleteMessages(lw, chat_id)
                await conn.execute('insert into welcomehistory (chat_id, time, cmid) values ($1, $2, $3) on conflict '
                                   '(chat_id) do nothing', chat_id, time.time(), m[0].conversation_message_id)
