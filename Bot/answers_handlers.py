...
async def queue_handler(event: MessageNew):
    ...
    if queue[3].startswith("notification_"):
        if queue[3] == "notification_text":
            ...
            await editMessage(
                messages.notification_changed_text(name),
            ...
            if ctype == "single":
                try:
                    if (
                        int(ctime.split(":")[0]) > 23 or int(ctime.split(":")[0]) < 0
                    ) or (
                ...
                nctime = datetime.datetime.now().replace(
                    hour=ctime.hour, minute=ctime.minute
                )
            ...
            cmid = int(additional["cmid"])
            await editMessage(
                messages.notification_changed_time(name),
                event.object.message.peer_id,
                cmid,
            )
            await sendMessage(
        ...
        if (
            queue[3] == "settings_change_countable"
            and additional["setting"] == "autodelete"
        ):
            ...
            if not (matches := re.findall(r"(\d+)\s*([hm])", text)):
                await editMessage(
                    messages.settings_autodelete_input_error(),
                    event.object.message.peer_id,
                    cmid,
            ...
            await editMessage(
                messages.settings_change_autodelete_done(itime),
                event.object.message.peer_id,
                cmid,
                keyboard.settings_change_countable(
                    uid, category, setting, onlybackbutton=True
            ...
            if setting not in SETTINGS_COUNTABLE_MULTIPLE_ARGUMENTS:
                ...
                if not text.isdigit() or int(text) not in limit:
                    await editMessage(
                        messages.settings_change_countable_digit_error(),
                        event.object.message.peer_id,
                        cmid,
                        kb,
                    )
                ...
                if setting == "nightmode":
                    ...
                    val += (
                        f"0{start.minute}-" if start.minute < 10 else f"{start.minute}-"
                    )
            ...
            await editMessage(
                messages.settings_change_countable_done(setting, text),
                event.object.message.peer_id,
                cmid,
                kb,
            )
            ...
            await editMessage(
                messages.settings_set_punishment(action, int(pnshtime)),
                event.object.message.peer_id,
                cmid,
                kb,
            )
            ...
            if queue[3] == "settings_set_welcome_text":
                ...
                if (
                    len(event.object.message.attachments) == 0
                    or event.object.message.attachments[0].type
                    != MessagesMessageAttachmentType.PHOTO
                ):
                    return await sendMessage(
                        chat_id + 2000000000,
                        messages.get(queue[3] + "_not_photo"),
                        keyboard.settings_change_countable(
                            uid, "main", "welcome", onlybackbutton=True
                        ),
                    )
                r = requests.get(
                    event.object.message.attachments[0].photo.sizes[-1].url
                )
                ...
                if welcome and not welcome[0] and not welcome[1]:
                    return await sendMessage(
                        chat_id + 2000000000,
                        messages.get(queue[3] + "_empty_text_url"),
                        keyboard.settings_change_countable(
                            uid, "main", "welcome", onlybackbutton=True
                        ),
                    )
                if len(" ".join(event.object.message.text.split()[1:])) > 40:
                    return await sendMessage(
                        chat_id + 2000000000,
                        messages.get(queue[3] + "_limit_text"),
                        keyboard.settings_change_countable(
                            uid, "main", "welcome", onlybackbutton=True
                        ),
                    )
                ...
                async with (await pool()).acquire() as conn:
                    if not await conn.fetchval(
                        "update welcome set url = $1, button_label = $2 where chat_id=$3 "
                        "returning 1",
                        text[-1],
                        " ".join(text[0:-1]),
                        chat_id,
            ...
            if setting == "disallowLinks":
                ...
                if action == "add":
                    ...
                    async with (await pool()).acquire() as conn:
                        if not await conn.fetchval(
                            "select exists(select 1 from antispamurlexceptions where "
                            "chat_id=$1 and url=$2)",
                            chat_id,
                            url,
                        ):
                ...
                await sendMessage(
                    chat_id + 2000000000,
                    messages.settings_listaction_done(setting, action, url),
                    keyboard.settings_change_countable(
                        uid, category, setting, onlybackbutton=True
                    ),
                )
            if setting == "vkLinks":
                ...
                if action == "add":
                    ...
                    async with (await pool()).acquire() as conn:
                        if not await conn.fetchval(
                            "select exists(select 1 from vklinksexceptions where "
                            "chat_id=$1 and url=$2)",
                            chat_id,
                            url,
                        ):
            ...
            if setting == "forwardeds":
                ...
                if action == "add":
                    ...
                    async with (await pool()).acquire() as conn:
                        if not await conn.fetchval(
                            "select exists(select 1 from forwardedsexceptions where "
                            "chat_id=$1 and exc_id=$2)",
                            chat_id,
                            exc_id,
                        ):
                            await conn.execute(
                ...
                await sendMessage(
                    chat_id + 2000000000,
                    messages.settings_listaction_done(setting, action, exc_id),
                    keyboard.settings_change_countable(
                        uid, category, setting, onlybackbutton=True
                    ),
                )
        ...
        if queue[3] == "premmenu_action_border_color":
            ...
            await sendMessage(
                chat_id + 2000000000,
                messages.premmenu_action_complete(
        ...
        if queue[3] == "prefix_add":
            ...
            async with (await pool()).acquire() as conn:
                if not await conn.fetchval(
                    ...
                    await sendMessage(
                        event.object.message.peer_id,
                        messages.delprefix_not_found(event.object.message.text),
                        keyboard.prefix_back(uid),
                    )
                    return
            await sendMessage(
