...
💬 Сообщение: \"{event.object.message.text}\"""",
    ...
    if (
        uacc == 0
        and settings["main"]["disallowPings"]
        and any(
            i
            in [
                "@all",
    ...
    try:
        if (
            event.object.message.attachments[0].type
    ...
    if uacc < 5 and (
        setting := await antispamChecker(chat_id, uid, event.object.message, settings)
    ):
        async with (await pool()).acquire() as conn:
        ...
        if (
            setting[1] in SETTINGS_ALT_TO_DELETE
            and (
                setting[3]
                or (setting[3] is None and SETTINGS_ALT()["antispam"][setting[1]])
            )
        ...
        async with (await pool()).acquire() as conn:
            ...
            if math and math[2] == int(msg):
                ...
                await sendMessage(
                    ...
                    messages.math_winner(
                        uid,
                        await getUserName(uid),
                        await getUserNickname(uid, MATHGIVEAWAYS_TO),
                        msg,
                        math[3],
                        math[4],
                    ),
