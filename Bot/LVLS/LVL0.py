...
async def id(message: Message):
    ...
    if id < 0:
        return await messagereply(
            message, disable_mentions=1, message=messages.id_group()
        )
        ...
        await messagereply(
            ...
            attachment=await uploadImage(
                await createStatsImage(
                    await getUserWarns(id, chat_id),
                    await getUserMessages(id, chat_id),
                    id,
                    ...
                    await getUserMute(id, chat_id),
                    await getUserBan(id, chat_id),
                    lvl_name or LVL_NAMES[acc],
                    await getUserNeededXP(xp) if lvl < 999 else 0,
...
async def help(message: Message):
    ...
    await messagereply(
        message,
        disable_mentions=1,
        message=messages.help(cmds=base),
...
async def bonus(message: Message):
    ...
    try:
        await tgbot.send_message(
            chat_id=TG_CHAT_ID,
            message_thread_id=TG_BONUS_THREAD_ID,
...
async def cmd(message: Message):
    ...
    if len(data) == 2:
        try:
            ...
            return await messagereply(
                message, disable_mentions=1, message=messages.resetcmd_hint()
            )
        if cmd not in COMMANDS:
            return await messagereply(
                message, disable_mentions=1, message=messages.resetcmd_not_found(cmd)
            )
        ...
        if not cmdn:
            return await messagereply(
                message, disable_mentions=1, message=messages.resetcmd_not_changed(cmd)
            )

        await messagereply(
            message,
            disable_mentions=1,
            message=messages.resetcmd(
                uid,
        ...
        try:
            ...
            return await messagereply(
                message, disable_mentions=1, message=messages.cmd_hint()
            )
        if cmd not in COMMANDS:
            return await messagereply(
                message, disable_mentions=1, message=messages.resetcmd_not_found(cmd)
        ...
        if changed in COMMANDS:
            return await messagereply(
                message, disable_mentions=1, message=messages.cmd_changed_in_cmds()
            )
        ...
        for i in cmdns:
            ...
            if changed == i[1]:
                return await messagereply(
                    message,
                    disable_mentions=1,
                    message=messages.cmd_changed_in_users_cmds(i[0]),
                )
        if (
            not await getUserPremium(uid)
            and len(res) >= CMDLEAGUES[await getUserLeague(uid) - 1]
        ):
            return await messagereply(
                message, disable_mentions=1, message=messages.cmd_prem(len(res))
            )
        ...
        if (
            ...
            return await messagereply(
                message, disable_mentions=1, message=messages.cmd_char_limit()
            )
...
async def premmenu(message: Message):
    ...
    await messagereply(
        message,
        disable_mentions=1,
        message=messages.premmenu(settings, prem),
...
async def duel(message: Message):
    ...
    if not (await getChatSettings(chat_id))["entertaining"]["allowDuel"]:
        return await messagereply(
            message, disable_mentions=1, message=messages.duel_not_allowed()
        )
...
async def transfer(message: Message):
    ...
    if id < 0:
        return await messagereply(
            message, disable_mentions=1, message=messages.transfer_community()
        )
    if not id:
        return await messagereply(
    ...
    if uid == id:
        return await messagereply(
            message, disable_mentions=1, message=messages.transfer_myself()
        )
    if await getULvlBanned(id):
        return await messagereply(
            message, disable_mentions=1, message=messages.user_lvlbanned()
        )
    ...
    if await getUserCoins(uid) < tcoins:
        return await messagereply(
            message,
            disable_mentions=1,
            message=messages.transfer_not_enough(
                uid, await getUserName(uid), await getUserNickname(uid, chat_id)
            ),
        )
...
async def start(message: Message):
    chat_id = message.peer_id - 2000000000
    if await getUserAccessLevel(message.from_id, chat_id) >= 7 or await isChatAdmin(
        message.from_id, chat_id
    ):
        await messagereply(
            message, messages.rejoin(), keyboard=keyboard.rejoin(chat_id)
        )
...
async def chats(message: Message):
    await messagereply(
        message, messages.chats(), keyboard=keyboard.chats(), disable_mentions=1
    )
...
async def guess(message: Message):
    if not (await getChatSettings(message.chat_id))["entertaining"]["allowGuess"]:
        return await messagereply(
            message, disable_mentions=1, message=messages.guess_not_allowed()
        )
    data = message.text.split()
    if (
        len(data) != 3
        ...
        return await messagereply(
            message, disable_mentions=1, message=messages.guess_hint()
        )
...
async def promo(message: Message):
    ...
    if len(data) != 2:
        return await messagereply(
            message, disable_mentions=1, message=messages.promo_hint()
        )
    uid = message.from_id
    async with (await pool()).acquire() as conn:
        ...
        if not code or await conn.fetchval(
            ...
            return await messagereply(
                message,
                disable_mentions=1,
                message=messages.promo_alreadyusedornotexists(
                    uid,
                    await getUserNickname(uid, message.chat_id),
                    await getUserName(uid),
                ),
            )
        await conn.execute(
...
async def rep(message: Message):
    ...
    if len(data) not in (2, 3) or data[1] not in ("+", "-") or not id or id < 0:
        return await messagereply(
            message, disable_mentions=1, message=messages.rep_hint()
        )
    if id == message.from_id:
        return await messagereply(
            message, disable_mentions=1, message=messages.rep_myself()
        )
    members = (
        await api.messages.get_conversation_members(peer_id=message.peer_id)
    ).items
    if len(members) < 2900 and id not in [i.member_id for i in members]:
        return await messagereply(
            message, disable_mentions=1, message=messages.rep_notinchat()
        )
    uid = message.from_id
    ...
    async with (await pool()).acquire() as conn:
        ...
        if len(rephistory) >= (3 if uprem else 1):
            return await messagereply(
                message,
                disable_mentions=1,
                message=messages.rep_limit(uprem, rephistory[0][0]),
            )
        if not await conn.fetchval(
    ...
    await messagereply(
        ...
        message=messages.rep(
            ...
            id,
            await getUserName(id),
            await getUserNickname(id, message.chat_id),
            await getUserRep(id),
            await getRepTop(id),
        ),
    )
...
async def short(message: Message):
    ...
    if len(data) != 2:
        return await messagereply(
            message, disable_mentions=1, message=messages.short_hint()
        )
    try:
        ...
        return await messagereply(
            message, disable_mentions=1, message=messages.short_failed()
        )
    await messagereply(
        message,
        disable_mentions=1,
...
async def rewards(message: Message):
    ...
    if not await api.groups.is_member(group_id=GROUP_ID, user_id=uid):
        return await messagereply(
            ...
            message=messages.rewards_unsubbed(
                uid, await getUserName(uid), await getUserNickname(uid, message.chat_id)
            ),
        )
    async with (await pool()).acquire() as conn:
        ...
        if collected:
            if collected[0]:
                ...
                return await messagereply(
                    message,
                    disable_mentions=1,
                    message=messages.rewards_activated(
                        uid,
                        await getUserName(uid),
                        await getUserNickname(uid, message.chat_id),
                        collected[1],
            ...
            return await messagereply(
                message,
                disable_mentions=1,
                message=messages.rewards_collected(
                    uid,
                    await getUserName(uid),
                    await getUserNickname(uid, message.chat_id),
