...
from Bot.dailyplan import update_names
...
async def reboot(message: Message):
    ...
    msg = messages.reboot()
    await message.reply(msg)
    os.system('pkill python && /root/startup.sh')
...
async def getuserchats(message: Message):
    ...
    if id is None:
        await message.reply('🔶 Пользователь не найден')
        return
    top = Messages.select().where(Messages.uid == id).order_by(Messages.messages.desc())
...
async def getchats(message: Message):
    top = Messages.select().order_by(Messages.messages.desc())
    msg = '✝ Беседы:\n'
    # edit = await message.reply(msg)
    b = []
    for i in top:
...
async def gettransferhistory(message: Message):
    ...
    if message.text.lower().split()[0][-2:] == 'to':
        ...
        transfers = (TransferHistory.select().where((TransferHistory.from_id == id) | (TransferHistory.to_id == id))
                     .order_by(TransferHistory.time.desc()).limit(100))
...
async def gettransferhistory(message: Message):
    threading.Thread(update_names()).start()
