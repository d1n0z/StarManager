    ...
    for i in chunks(chats, 2500):
        try:
            ...
            for y in chunks(i, 100):
                code += (
                    'API.messages.send({"random_id": 0, "peer_ids": ['
...
async def block(message: Message):
    ...
    if data[1] == "chat":
        await sendMessage(
            id + 2000000000,
            messages.block_chatblocked(id, reason),
            keyboard.block_chatblocked(),
        )
        await api.messages.remove_chat_user(id, member_id=-GROUP_ID)
    else:
        await sendMessage(
            id, messages.block_userblocked(id, reason), keyboard.block_chatblocked()
        )
        for i in chats:
            if await kickUser(id, i):
                await sendMessage(
                    i + 2000000000,
                    messages.block_blockeduserinvite(id, await getUserName(id), reason),
                )
        ...
        await messagereply(
            ...
            (
                await api.messages.get_invite_link(
                    peer_id=int(data[1]) + 2000000000, reset=0, group_id=GROUP_ID
                )
            ).link,
        )
    except Exception:
...
async def cmdstats(message: Message):
    ...
    await messagereply(
        message,
        f"Использований: {pointWords(len(c), ('раз', 'раза', 'раз'))}.\n"
...
async def prempromocreate(message: Message):
    ...
    async with (await pool()).acquire() as conn:
        if await conn.fetchval(
            ...
            return await messagereply(
                message, messages.prempromocreate_alreadyexists(data[1])
            )
        await conn.execute(
