...
async def timeout(message: Message):
    ...
    await messagereply(
        message,
        disable_mentions=1,
        message=messages.timeout(activated),
...
async def inactive(message: Message):
    ...
    if len(data) != 2:
        return await messagereply(
            message, disable_mentions=1, message=messages.inactive_hint()
        )
    count = data[1]
    ...
    if len(res) <= 0:
        return await messagereply(
            message, disable_mentions=1, message=messages.inactive_no_results()
        )
...
async def ban(message: Message):
    ...
    if (len(data) == 1 and message.reply_message is None) or not id:
        return await messagereply(
            message, disable_mentions=1, message=messages.ban_hint()
        )
    if id < 0:
        return await messagereply(
    ...
    if await getUserAccessLevel(id, chat_id) >= await getUserAccessLevel(uid, chat_id):
        return await messagereply(
            message, disable_mentions=1, message=messages.ban_higher()
        )
    ...
    if (ch_ban := await getUserBan(id, chat_id)) >= time.time():
        return await messagereply(
            message,
            disable_mentions=1,
            message=messages.already_banned(
...
async def unban(message: Message):
    ...
    if (len(data) == 1 and message.reply_message is None) or not id:
        return await messagereply(
            message, disable_mentions=1, message=messages.unban_hint()
        )
    if id < 0:
        return await messagereply(
            message, disable_mentions=1, message=messages.id_group()
    ...
    if await getUserAccessLevel(id, chat_id) >= await getUserAccessLevel(uid, chat_id):
        return await messagereply(
            message, disable_mentions=1, message=messages.unban_higher()
        )
    if await getUserBan(id, chat_id) <= time.time():
        return await messagereply(
            message,
            disable_mentions=1,
            message=messages.unban_no_ban(
...
async def zov(message: Message):
    ...
    if len(data) <= 1:
        return await messagereply(
            message, disable_mentions=1, message=messages.zov_hint()
        )
    await messagereply(
        message,
        message=messages.zov(
            uid,
            await getUserName(uid),
