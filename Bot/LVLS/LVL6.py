...
async def gdelaccess(message: Message):
    ...
    if not id:
        return await messagereply(
            message, disable_mentions=1, message=messages.gdelaccess_hint()
        )
    if id < 0:
        return await messagereply(
    ...
    if uid == id:
        return await messagereply(
            message, disable_mentions=1, message=messages.delaccess_myself()
        )
    if not (chats := await getgpool(chat_id)):
        return await messagereply(
            message, disable_mentions=1, message=messages.chat_unbound()
        )
    ...
    edit = await messagereply(
        message,
        disable_mentions=1,
        message=messages.gdelaccess_start(
            uid,
    ...
    for chat_id in chats:
        ...
        if (
            ...
            or await getUserAccessLevel(id, chat_id) >= u_acc
        ):
            continue
    ...
    await api.messages.edit(
        ...
        message=messages.gdelaccess(
            id,
            name,
            await getUserNickname(id, edit.peer_id - 2000000000),
            len(chats),
            success,
        ),
    )
...
async def gsetaccess(message: Message):
    ...
    if (len(data) <= 2 and message.reply_message is None) or (
        ...
        return await messagereply(
            message, message=messages.gsetaccess_hint(), disable_mentions=1
        )
    id = await getIDFromMessage(message.text, message.reply_message)
    if id < 0:
        return await messagereply(
    ...
    if uid == id:
        return await messagereply(
            message, disable_mentions=1, message=messages.setaccess_myself()
        )
    ...
    try:
        ...
        return await messagereply(
            message, disable_mentions=1, message=messages.gsetaccess_hint()
        )
    if not (chats := await getgpool(chat_id)):
        return await messagereply(
            message, disable_mentions=1, message=messages.chat_unbound()
    ...
    edit = await messagereply(
        message,
        disable_mentions=1,
        message=messages.gsetaccess_start(
            uid,
            await getUserName(uid),
            await getUserNickname(uid, chat_id),
            id,
...
async def demote(message: Message):
    await messagereply(
        message,
        disable_mentions=1,
        message=messages.demote_choose(),
...
async def ssetaccess(message: Message):
    ...
    if (len(data) <= 3 and message.reply_message is None) or (
        ...
        return await messagereply(
            message, message=messages.ssetaccess_hint(), disable_mentions=1
        )
    ...
    try:
        ...
        return await messagereply(
            message, disable_mentions=1, message=messages.setacc_hint()
        )

    if not (chats := await getpool(chat_id, data[1])):
        return await messagereply(
            message, disable_mentions=1, message=messages.s_invalid_group(data[1])
        )
    ...
    edit = await messagereply(
        message,
        disable_mentions=1,
        message=messages.ssetaccess_start(
            uid,
            await getUserName(uid),
            await getUserNickname(uid, chat_id),
            id,
...
async def sdelaccess(message: Message):
    ...
    if (len(data) <= 2 and message.reply_message is None) or (
        ...
        return await messagereply(
            message, message=messages.sdelaccess_hint(), disable_mentions=1
        )
    id = await getIDFromMessage(message.text, message.reply_message, 3)
    if not id:
        return await messagereply(
            message, disable_mentions=1, message=messages.sdelaccess_hint()
        )
    if id < 0:
        return await messagereply(
            message, disable_mentions=1, message=messages.id_group()
    ...
    edit = await messagereply(
        message,
        disable_mentions=1,
        message=messages.sdelaccess_start(
            uid,
            await getUserName(uid),
            await getUserNickname(uid, chat_id),
            id,
...
async def ignore(message: Message):
    ...
    if not id:
        return await messagereply(
            message, disable_mentions=1, message=messages.ignore_hint()
        )
    if await getUserAccessLevel(uid, chat_id) <= await getUserAccessLevel(id, chat_id):
        return await messagereply(
            message, disable_mentions=1, message=messages.ignore_higher()
        )
    if id < 0:
        return await messagereply(
            message, disable_mentions=1, message=messages.id_group()
...
async def unignore(message: Message):
    ...
    async with (await pool()).acquire() as conn:
        if not await conn.fetchval(
            ...
            return await messagereply(
                message, disable_mentions=1, message=messages.unignore_not_ignored()
            )
    await messagereply(
        message,
        disable_mentions=1,
        message=messages.unignore(id, name, await getUserNickname(uid, chat_id)),
    )
...
async def ignorelist(message: Message):
    ...
    await messagereply(
        message, disable_mentions=1, message=messages.ignorelist(ids, names)
    )
...
async def chatlimit(message: Message):
    ...
    if len(data) != 2:
        return await messagereply(
            message, disable_mentions=1, message=messages.chatlimit_hint()
        )
...
async def resetnick(message: Message):
    await messagereply(
        message,
        disable_mentions=1,
        message=messages.resetnick_yon(),
...
async def resetaccess(message: Message):
    ...
    if len(data) != 2 or not data[1].isdigit() or int(data[1]) < 1 or int(data[1]) > 6:
        return await messagereply(
            message, disable_mentions=1, message=messages.resetaccess_hint()
        )
    await messagereply(
        message,
        disable_mentions=1,
        message=messages.resetaccess_yon(
            await getChatAccessName(message.chat_id, int(data[1]))
            or LVL_NAMES[int(data[1])]
        ),
...
async def notif(message: Message):
        ...
        async with (await pool()).acquire() as conn:
            ...
            if not notif:
                ...
                return await messagereply(
                    message,
                    disable_mentions=1,
                    message=messages.notification(name, "", time.time(), -1, 1, 1),
                    keyboard=keyboard.notification(uid, 1, name),
                )
        return await messagereply(
            message, disable_mentions=1, message=messages.notif_already_exist(name)
        )
    async with (await pool()).acquire() as conn:
    ...
    await messagereply(
        message,
        disable_mentions=1,
        message=messages.notif(notifs, activenotifs),
...
async def purge(message: Message):
    chat_id = message.peer_id - 2000000000
    edit = await messagereply(
        message, disable_mentions=1, message=messages.purge_start()
    )
    users = [
        i.member_id
        for i in (
    ...
    async with (await pool()).acquire() as conn:
        ...
        for i in await conn.fetch(
            ...
            if i[1] not in users:
                ...
                dtdaccesslevels += 1
    await editMessage(
        messages.purge(dtdnicknames, dtdaccesslevels)
        if dtdnicknames > 0 or dtdaccesslevels > 0
        else messages.purge_empty(),
        message.peer_id,
...
async def rename(message: Message):
    ...
    if len(data) < 2:
        return await messagereply(
            message, disable_mentions=1, message=messages.rename_hint()
        )
    if len(" ".join(data[1:])) >= 100:
        return await messagereply(
            message, disable_mentions=1, message=messages.rename_toolong()
        )
    ...
    try:
        ...
        return await messagereply(
            message, disable_mentions=1, message=messages.rename_error()
        )
    async with (await pool()).acquire() as conn:
    ...
    await messagereply(
        ...
        message=messages.rename(
            message.from_id,
            await getUserName(message.from_id),
