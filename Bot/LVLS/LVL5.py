...
async def skick(message: Message):
    ...
    if not id or len(data) < 2:
        return await messagereply(
            message, disable_mentions=1, message=messages.skick_hint()
        )
    if id < 0:
        return await messagereply(
...
async def sban(message: Message):
    ...
    if not id or len(data) < 2:
        return await messagereply(
            message, disable_mentions=1, message=messages.sban_hint()
        )
    if id < 0:
        return await messagereply(
            message, disable_mentions=1, message=messages.id_group()
    ...
    if uid == id:
        return await messagereply(
            message, disable_mentions=1, message=messages.ban_myself()
        )
...
async def sunban(message: Message):
    ...
    if not id or len(data) < 2:
        return await messagereply(
            message, disable_mentions=1, message=messages.sunban_hint()
        )
    if id < 0:
        return await messagereply(
            message, disable_mentions=1, message=messages.id_group()
    ...
    if uid == id:
        return await messagereply(
            message, disable_mentions=1, message=messages.unban_myself()
        )
    ...
    for chat_id in chats:
        ...
        if (
            ...
            or await getUserAccessLevel(id, chat_id) >= u_acc
            or await getUserBan(id, chat_id) <= time.time()
        ):
            continue
        async with (await pool()).acquire() as conn:
...
async def ssnick(message: Message):
    ...
    if not id or (
        ...
        return await messagereply(
            message, disable_mentions=1, message=messages.ssnick_hint()
        )
    if id < 0:
        return await messagereply(
            message, disable_mentions=1, message=messages.id_group()
    ...
    for chat_id in chats:
        ...
        if not await haveAccess("ssnick", chat_id, u_acc) or (
            u_acc <= await getUserAccessLevel(id, chat_id) and uid != id
        ):
            continue
        async with (await pool()).acquire() as conn:
        ...
        await sendMessage(
            chat_id + 2000000000,
            messages.snick(uid, u_name, u_nick, id, name, ch_nick, nickname),
        )
        success += 1
...
async def srnick(message: Message):
    ...
    if not id or len(data) < 2:
        return await messagereply(
            message, disable_mentions=1, message=messages.srnick_hint()
        )
    if id < 0:
        return await messagereply(
            message, disable_mentions=1, message=messages.id_group()
...
async def szov(message: Message):
    ...
    if len(data) < 3:
        return await messagereply(
            message, disable_mentions=1, message=messages.szov_hint()
        )
    ...
    edit = await messagereply(
        message,
        disable_mentions=1,
        message=messages.szov_start(uid, name, nickname, len(chats), data[1]),
    )
    success = 0
    text = " ".join(data[2:])
    for chat_id in chats:
        if not await haveAccess(
            "szov", chat_id, await getUserAccessLevel(uid, chat_id)
        ):
            continue
        try:
    ...
    await api.messages.edit(
        ...
        message=messages.szov(
            uid,
            name,
            await getUserNickname(uid, edit.peer_id - 2000000000),
            data[1],
            len(chats),
            success,
        ),
...
async def chat(message: Message):
    ...
    await messagereply(
        ...
        keyboard=None
        if not await haveAccess(
            "settings", chat_id, await getUserAccessLevel(message.from_id, chat_id)
        )
...
async def antitag(message: Message):
    ...
    if (
        ...
        or not (id := await getIDFromMessage(message.text, message.reply_message, 3))
    ):
        return await messagereply(
            message,
            disable_mentions=1,
            message=messages.antitag(),
...
async def pin(message: Message):
    if not message.reply_message:
        return await messagereply(
            message, disable_mentions=1, message=messages.pin_hint()
        )
    try:
        ...
        return await messagereply(
            message, disable_mentions=1, message=messages.pin_cannot()
        )
    else:
...
async def unpin(message: Message):
    if (
        not (await api.messages.get_conversations_by_id(peer_ids=message.peer_id))
        .items[0]
        .chat_settings.pinned_message
    ):
        return await messagereply(
            message, disable_mentions=1, message=messages.unpin_notpinned()
        )
    try:
        ...
        return await messagereply(
            message, disable_mentions=1, message=messages.unpin_cannot()
        )
    else:
        await deleteMessages(message.conversation_message_id, message.chat_id)
