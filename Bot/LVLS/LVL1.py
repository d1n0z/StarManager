...
async def kick(message: Message):
    ...
    if id == 0:
        return await messagereply(
            message, disable_mentions=1, message=messages.kick_hint()
        )
    if id == uid:
        return await messagereply(
            message, disable_mentions=1, message=messages.kick_myself()
        )
    ...
    if await getUserAccessLevel(id, chat_id) >= uacc:
        return await messagereply(
            message, disable_mentions=1, message=messages.kick_higher()
        )
...
async def mkick(message: Message):
    ...
    if not ids:
        return await messagereply(
            message, disable_mentions=1, message=messages.mkick_error()
        )
    ...
    for ind, id in enumerate(ids):
        if u_acc <= await getUserAccessLevel(id, chat_id) or not await kickUser(
            id, chat_id
        ):
            continue
...
async def mute(message: Message):
    ...
    if not id:
        return await messagereply(
            message, disable_mentions=1, message=messages.mute_hint()
        )
    if id < 0:
        return await messagereply(
    ...
    if id == uid:
        return await messagereply(
            message, disable_mentions=1, message=messages.mute_myself()
        )
    ...
    if await getUserAccessLevel(id, chat_id) >= await getUserAccessLevel(uid, chat_id):
        return await messagereply(
            message, disable_mentions=1, message=messages.mute_higher()
        )
    ...
    if (ch_mute := await getUserMute(id, chat_id)) >= time.time():
        return await messagereply(
            message,
            disable_mentions=1,
            message=messages.already_muted(
...
async def warn(message: Message):
    ...
    if not id or (len(data) < 2 and message.reply_message is None):
        return await messagereply(
            message, disable_mentions=1, message=messages.warn_hint()
        )
    if id < 0:
        return await messagereply(
            message, disable_mentions=1, message=messages.id_group()
    ...
    if id == uid:
        return await messagereply(
            message, disable_mentions=1, message=messages.warn_myself()
        )
    if await getUserAccessLevel(id, chat_id) >= await getUserAccessLevel(uid, chat_id):
        return await messagereply(
            message, disable_mentions=1, message=messages.warn_higher()
        )
...
async def clear(message: Message):
    ...
    if not deleting:
        return await messagereply(
            message, disable_mentions=1, message=messages.clear_hint()
        )
    deleting = [
        i
        for i in deleting
        if await getUserAccessLevel(i[0], chat_id)
        <= await getUserAccessLevel(uid, chat_id)
    ]
    if deleting:
        try:
            ...
            if all(
                True if i.error and "admin message" in i.error.description else False
                for i in deleted
            ):
                return await messagereply(
                    message, disable_mentions=1, message=messages.clear_admin()
                )
            if all(True if i.error else False for i in deleted):
                return await messagereply(
                    message, disable_mentions=1, message=messages.clear_old()
                )
...
async def snick(message: Message):
    ...
    if not id or len(data) <= 1:
        return await messagereply(
            message, disable_mentions=1, message=messages.snick_hint()
        )
    if id < 0:
        return await messagereply(
            message, disable_mentions=1, message=messages.id_group()
    ...
    if len(nickname) >= 46 or "[" in nickname or "]" in nickname:
        return await messagereply(
            message, disable_mentions=1, message=messages.snick_too_long_nickname()
        )

    if await getUserAccessLevel(id, chat_id) > await getUserAccessLevel(uid, chat_id):
        return await messagereply(
            message, disable_mentions=1, message=messages.snick_higher()
        )
...
async def rnick(message: Message):
    ...
    if await getUserAccessLevel(id, chat_id) > await getUserAccessLevel(uid, chat_id):
        return await messagereply(
            message, disable_mentions=1, message=messages.rnick_higher()
        )
    if not (ch_nickname := await getUserNickname(id, chat_id)):
        return await messagereply(
            message, disable_mentions=1, message=messages.rnick_user_has_no_nick()
        )
...
async def nlist(message: Message):
    ...
    await messagereply(
        message,
        disable_mentions=1,
        message=messages.nlist(res, await api.users.get(user_ids=[i[0] for i in res])),
...
async def getnick(message: Message):
    ...
    if len(data) <= 1:
        return await messagereply(
            message, disable_mentions=1, message=messages.getnick_hint()
        )
    query = " ".join(data[1:])
    async with (await pool()).acquire() as conn:
        res = await conn.fetch(
    ...
    if not res:
        return await messagereply(
            message, disable_mentions=1, message=messages.getnick_no_result(query)
        )
    await messagereply(
...
async def olist(message: Message):
    ...
    await messagereply(
        message, disable_mentions=1, message=messages.olist(members_online)
    )
...
async def check(message: Message):
    ...
    if not id:
        return await messagereply(
            message, disable_mentions=1, message=messages.check_help()
        )
    if id < 0:
        return await messagereply(
            message, disable_mentions=1, message=messages.id_group()
...
async def scan(message: Message):
    ...
    if len(data) < 2 and not message.reply_message:
        return await messagereply(
            message, disable_mentions=1, message=messages.scan_hint()
        )
    links = set()
    for i in (
