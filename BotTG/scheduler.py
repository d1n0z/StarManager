...
async def new():
    try:
        msg = await bot.send_message(
            chat_id=config.TG_PUBLIC_CHAT_ID,
            message_thread_id=config.TG_PUBLIC_GIVEAWAY_THREAD_ID,
        ...
        async with (await pool()).acquire() as conn:
            async with conn.transaction():
                await conn.execute(
                    "insert into tggiveaways (mid) values ($1)", msg.message_id
                )
    except Exception:
        traceback.print_exc()
...
async def end():
    try:
        ...
        await bot.edit_message_text(
            chat_id=config.TG_PUBLIC_CHAT_ID, message_id=mid, text=text
        )
    except Exception:
        traceback.print_exc()
