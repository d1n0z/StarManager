...
async def joingiveaway(query: CallbackQuery, state: FSMContext):
    try:
        member = await query.bot.get_chat_member(
            chat_id=config.TG_PUBLIC_CHAT_ID, user_id=query.from_user.id
        )
        if not isinstance(
    ...
    async with (await pool()).acquire() as conn:
        async with conn.transaction():
            ...
            vkid = await conn.fetchval(
                "select vkid from tglink where tgid=$1", message.from_user.id
            )
...
async def link(message: Message, state: FSMContext):
    ...
    msg = await message.bot.send_message(
        chat_id=message.from_user.id, reply_markup=keyboard.back(), text=text
    )
    await state.update_data(msg=msg)
